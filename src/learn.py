...
def main():
    encoder_conf = open("../train/encoder.json", "r")
    encoder_layers = json.load(encoder_conf)
    decoder_conf = open("../train/decoder.json", "r")
    ...
    z_mean = tf.cond(pred=is_train, true_fn=lambda: z_post_mean, false_fn=lambda: z_prior_mean)
    z_log_var = tf.cond(pred=is_train, true_fn=lambda: z_post_log_var, false_fn=lambda: z_prior_log_var)
    ...
    with tf.compat.v1.name_scope('loss'):
        ...
        latent_loss = -tf.reduce_mean(input_tensor=1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
        # what is the appropriate beta...
        loss = recon_loss + beta * latent_loss
        
    train_step = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(loss, global_step=global_step)
