...
fc = tf_slim.layers.fully_connected
...
def make_cells(n_layers, n_units):
    ...
    for j in range(n_layers):
        with tf.compat.v1.variable_scope("layer_{}".format(j), reuse=False):
...
def image_processor(h, layers):
    ...
    with tf.compat.v1.variable_scope("visual_feature_extractor"):
...
def generation(x, z_mean, z_log_var,
    ...
    eps = tf.random.normal(z_mean.get_shape().as_list(),
